...
class ResponseValidators:
    ...
    def should_be_correct_body(self, response: dict):
        missing_keys = (
            self.card_request_environment.EXPECTED_TOP_LEVEL_ELEMENTS_GET_CARD
            - set(response.json())
        )
        assert (
            not missing_keys
    ...
    def should_be_correct_status_code(
        self, response: dict, endpoint_name: str, role: str, rbac_func: dict
    ):
